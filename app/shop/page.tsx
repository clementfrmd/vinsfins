    ...
    <div>
      <section className="relative h-[60vh] sm:h-[70vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center text-cream px-4 sm:px-6">
          <p className="uppercase tracking-luxury text-gold/70 text-[11px] mb-4 sm:mb-6">{t("shop.heroLabel")}</p>
          <h1 className="font-playfair text-3xl sm:text-5xl lg:text-7xl font-normal mb-4 sm:mb-6">{t("shop.heroTitle")}</h1>
          <p className="text-cream/45 max-w-md mx-auto font-light leading-relaxed text-sm sm:text-base">{t("shop.heroDesc")}</p>
        </div>
      </section>
      ...
      <section className="section-padding">
        <div className="container-custom mx-auto">
          <div className="flex gap-3 justify-start lg:justify-center mb-10 sm:mb-16 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:flex-wrap">
            <button onClick={() => setActiveCategory("all")} className={`px-5 py-3 text-[11px] tracking-luxury uppercase transition-all duration-500 whitespace-nowrap flex-shrink-0 min-h-[44px] ${activeCategory === "all" ? "bg-charcoal text-cream" : "text-charcoal/40 hover:text-charcoal"}`}>{t("shop.all")}</button>
            {categories.map((cat) => (
              <button key={cat.id} onClick={() => setActiveCategory(cat.id)} className={`px-5 py-3 text-[11px] tracking-luxury uppercase transition-all duration-500 whitespace-nowrap flex-shrink-0 min-h-[44px] ${activeCategory === cat.id ? "bg-charcoal text-cream" : "text-charcoal/40 hover:text-charcoal"}`}>{cat.label}</button>
            ))}
            <button onClick={() => setActiveCategory("gift")} className={`px-5 py-3 text-[11px] tracking-luxury uppercase transition-all duration-500 whitespace-nowrap flex-shrink-0 min-h-[44px] ${activeCategory === "gift" ? "bg-charcoal text-cream" : "text-charcoal/40 hover:text-charcoal"}`}>{t("shop.giftBoxes")}</button>
          </div>
            ...
            {filtered.map((wine) => (
              <div key={wine.id} className="wine-card group flex flex-col">
                ...
                <div className="flex flex-col flex-1">
                  {wine.isGiftBox && <span className="text-[10px] tracking-luxury uppercase text-gold mb-2">{t("shop.giftBox")}</span>}
                  <p className="text-[10px] uppercase tracking-luxury text-charcoal/30 mb-2">{wine.region}</p>
                  <Link href={`/shop/${wine.id}`}><h3 className="font-playfair text-lg mb-1 hover:opacity-60 transition-opacity duration-500">{wine.name}</h3></Link>
                  ...
                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-charcoal/5">
                    ...
                    <button onClick={() => addToCart(wine)} className="text-[10px] tracking-luxury uppercase bg-charcoal text-cream px-4 py-3 min-h-[44px] hover:bg-burgundy transition-colors duration-500">{t("shop.addToCart")}</button>
                  </div>
                </div>
              </div>
            ))}
