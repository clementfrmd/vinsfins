    ...
    <div className="bg-cream dark:bg-noir">
      <section className="relative h-[40vh] sm:h-[50vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center text-cream">
          <h1 className="font-caveat text-4xl sm:text-6xl lg:text-7xl font-normal">{t("shop.heroTitle")}</h1>
        </div>
      </section>
      ...
      <section className="px-4 sm:px-8 lg:px-20 py-12 sm:py-20 lg:py-32">
        <div className="max-w-7xl mx-auto">
          <div className="flex gap-3 justify-start lg:justify-center mb-10 sm:mb-16 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:flex-wrap">
            ...
            <button onClick={() => setActiveCategory("gift")} className={`px-4 py-2 text-[10px] tracking-luxury uppercase transition-all duration-500 whitespace-nowrap flex-shrink-0 min-h-[44px] ${activeCategory === "gift" ? "text-charcoal dark:text-cream border-b border-charcoal dark:border-cream" : "text-charcoal/80 dark:text-cream/60 hover:text-charcoal dark:hover:text-cream"}`}>{t("shop.giftBoxes")}</button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-4 sm:gap-x-6 gap-y-10 sm:gap-y-14">
            {filtered.map((wine) => (
              <div key={wine.id} className="group flex flex-col">
                ...
                <Link href={`/shop/${wine.id}`}><h3 className="font-playfair text-base mb-1 text-charcoal dark:text-cream hover:opacity-50 transition-opacity duration-500">{wine.name}</h3></Link>
                <div className="flex items-center justify-between mt-auto pt-3 border-t border-charcoal/5 dark:border-cream/10">
                  <span className="text-sm text-charcoal dark:text-cream">€{wine.priceShop}</span>
                  <button onClick={() => addToCart(wine)} className="text-[10px] tracking-luxury uppercase text-red-700 dark:text-red-500 hover:text-red-900 dark:hover:text-red-400 transition-colors duration-500 min-h-[44px] flex items-center">{t("shop.addToCart")}</button>
                </div>
              </div>
            ))}
          </div>
