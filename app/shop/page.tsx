      ...
      <section className="px-4 sm:px-8 lg:px-20 py-12 sm:py-20 lg:py-32">
        <div className="max-w-7xl mx-auto">
          <div className="flex gap-3 justify-start lg:justify-center mb-10 sm:mb-16 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:flex-wrap">
            <button onClick={() => setActiveCategory("all")} className={`px-4 py-2 text-[10px] tracking-luxury uppercase transition-all duration-500 whitespace-nowrap flex-shrink-0 min-h-[44px] ${activeCategory === "all" ? "text-charcoal border-b border-charcoal" : "text-charcoal/25 hover:text-charcoal/50"}`}>{t("shop.all")}</button>
            {categories.map((cat) => (
            ...
            <button onClick={() => setActiveCategory("gift")} className={`px-4 py-2 text-[10px] tracking-luxury uppercase transition-all duration-500 whitespace-nowrap flex-shrink-0 min-h-[44px] ${activeCategory === "gift" ? "text-charcoal border-b border-charcoal" : "text-charcoal/25 hover:text-charcoal/50"}`}>{t("shop.giftBoxes")}</button>
          </div>
            ...
            {filtered.map((wine) => (
              <div key={wine.id} className="group flex flex-col">
                ...
                <div className="flex items-center justify-between mt-auto pt-3 border-t border-charcoal/5">
                  ...
                  <button onClick={() => addToCart(wine)} className="text-[10px] tracking-luxury uppercase text-charcoal/30 hover:text-charcoal transition-colors duration-500">{t("shop.addToCart")}</button>
                </div>
              </div>
            ))}
          </div>
