    ...
    <div>
      <section className="relative h-[60vh] sm:h-[70vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center text-cream px-4 sm:px-6">
          <p className="uppercase tracking-luxury text-gold/70 text-[11px] mb-4 sm:mb-6">{t("wines.heroLabel")}</p>
          <h1 className="font-playfair text-3xl sm:text-5xl lg:text-7xl font-normal">{t("wines.heroTitle")}</h1>
        </div>
      </section>
        ...
        <div className="container-custom mx-auto">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6 mb-10 sm:mb-16 pb-6 sm:pb-8 border-b border-charcoal/5">
            <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:flex-wrap w-full sm:w-auto">
              <button onClick={() => setActiveCategory("all")} className={`px-5 py-3 text-[11px] tracking-luxury uppercase transition-all duration-500 whitespace-nowrap flex-shrink-0 min-h-[44px] ${activeCategory === "all" ? "bg-charcoal text-cream" : "text-charcoal/40 hover:text-charcoal"}`}>{t("wines.allWines")}</button>
              {categories.map((cat) => (
            ...
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-transparent border-b border-charcoal/15 px-0 py-2 text-xs text-charcoal/50 focus:outline-none focus:border-charcoal/30 tracking-wide min-h-[44px]">
              <option value="name">{t("wines.sortName")}</option>
              <option value="price-asc">{t("wines.sortPriceAsc")}</option>
              <option value="price-desc">{t("wines.sortPriceDesc")}</option>
              <option value="region">{t("wines.sortRegion")}</option>
            </select>
          </div>
              ...
              <div key={wine.id} className="wine-card group">
                ...
                <div className="flex items-center justify-between pt-4 sm:pt-5 border-t border-charcoal/5">
                  <div>
                    ...
                    <span className="text-charcoal/30 text-xs ml-1 font-light">/ {t("wines.glass")}</span>
                  </div>
                  <div>
                    <span className="text-charcoal/60">€{wine.priceBottle}</span>
                    <span className="text-charcoal/30 text-xs ml-1 font-light">/ {t("wines.bottle")}</span>
                  </div>
                </div>
              </div>
            ))}
