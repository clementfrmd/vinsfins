    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-sm bg-cream z-50 flex flex-col animate-slide-in-right">
        <div className="flex items-center justify-between p-6 border-b border-charcoal/5">
          ...
          <button onClick={() => setIsCartOpen(false)} className="text-charcoal/30 hover:text-charcoal min-w-[44px] min-h-[44px] flex items-center justify-center">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {items.length === 0 ? (
            <div className="text-center py-20">
              <p className="text-sm text-charcoal/30 font-light mb-6">{t("cartSidebar.empty")}</p>
              <Link href="/shop" onClick={() => setIsCartOpen(false)} className="text-[10px] tracking-luxury uppercase text-charcoal/40 hover:text-charcoal border-b border-charcoal/15 pb-1 transition-colors duration-500">{t("cartSidebar.browseWines")}</Link>
            </div>
          ) : (
              ...
              {items.map((item) => (
                <div key={item.wine.id} className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-charcoal/30">{item.wine.vintage}</p>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <button onClick={() => updateQuantity(item.wine.id, item.quantity + 1)} className="w-7 h-7 border border-charcoal/10 flex items-center justify-center text-xs hover:border-charcoal/30 transition-colors">+</button>
                      <button onClick={() => removeFromCart(item.wine.id)} className="ml-auto text-charcoal/20 hover:text-charcoal/50 transition-colors text-[10px] tracking-wider uppercase">{t("cart.remove")}</button>
                    </div>
                  </div>
                </div>
              ))}
        ...
        {items.length > 0 && (
          <div className="border-t border-charcoal/5 p-6 space-y-4">
            <div className="flex justify-between items-baseline">
              <span className="text-[10px] uppercase tracking-luxury text-charcoal/30">{t("cartSidebar.total")}</span>
