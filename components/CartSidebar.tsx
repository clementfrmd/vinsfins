    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-cream z-50 shadow-2xl flex flex-col animate-slide-in-right">
        <div className="flex items-center justify-between p-5 sm:p-6 border-b border-burgundy/10">
          <h2 className="font-playfair text-xl text-burgundy">{t("cartSidebar.title")}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6">
          {items.length === 0 ? (
            <div className="text-center py-20 px-4">
              <svg className="w-16 h-16 mx-auto mb-6 text-burgundy/15" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={0.75} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
              <p className="font-playfair text-xl text-charcoal/70 mb-3">{t("cartSidebar.empty")}</p>
              <p className="text-sm text-charcoal/35 font-light leading-relaxed mb-8">{t("cartSidebar.emptyDesc")}</p>
              <Link href="/shop" onClick={() => setIsCartOpen(false)} className="btn-outline text-[11px] inline-block">{t("cartSidebar.browseWines")}</Link>
            </div>
          ) : (
                ...
                <div key={item.wine.id} className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      <button onClick={() => updateQuantity(item.wine.id, item.quantity - 1)} className="w-[44px] h-[44px] sm:w-8 sm:h-8 rounded border border-charcoal/20 flex items-center justify-center text-sm hover:bg-burgundy hover:text-cream hover:border-burgundy transition-colors">−</button>
                      <span className="text-sm font-semibold w-6 text-center">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.wine.id, item.quantity + 1)} className="w-[44px] h-[44px] sm:w-8 sm:h-8 rounded border border-charcoal/20 flex items-center justify-center text-sm hover:bg-burgundy hover:text-cream hover:border-burgundy transition-colors">+</button>
                      <button onClick={() => removeFromCart(item.wine.id)} className="ml-auto text-charcoal/40 hover:text-red-500 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center">
        ...
        {items.length > 0 && (
          <div className="border-t border-burgundy/10 p-5 sm:p-6 space-y-4">
            <p className="text-xs text-charcoal/40 font-light">{t("cartSidebar.shippingNote")}</p>
            <div className="flex justify-between items-baseline border-t border-burgundy/5 pt-4">
              <span className="text-[11px] uppercase tracking-luxury text-charcoal/50">{t("cartSidebar.total")}</span>
            ...
            <Link href="/shop/cart" onClick={() => setIsCartOpen(false)} className="btn-outline block text-center w-full">{t("cartSidebar.viewCart")}</Link>
          </div>
        )}
      </div>
    </>
