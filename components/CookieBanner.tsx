    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-charcoal text-cream p-4 sm:p-6">
      <div className="container-custom mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex gap-3 flex-shrink-0">
          <button onClick={accept} className="btn-gold text-xs py-2 px-6">{t("cookie.accept")}</button>
          <button onClick={accept} className="text-cream/60 hover:text-cream text-xs font-semibold uppercase tracking-wide">{t("cookie.decline")}</button>
        </div>
      </div>
    </div>
  );
